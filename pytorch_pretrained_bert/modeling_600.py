...
class CapsuleNet(nn.Module):
    ...
    def __init__(self,seq_len,hidden_size,kernel_conv_h):
        ...
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES , num_route_nodes=16 * (seq_len-kernel_conv_h+1), in_channels=8,  # 修改！
                                           out_channels=16)
        self.mark=100
    ...
    def forward(self, x):
       # print("sequence_output:",list(x.size()))
        ...
        #print("reshape:", list(x.size()))
       #method2
        #x = x.view(x.size(0),x.size(1),x.size(2)).transpose(1,2).contiguous()
        ...
        if self.mark:
          # print("classes:",classes)
            print("classes = (x ** 2).sum(dim=-1)",list(classes.size()))
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None,device="cpu",is_eval=False):
        ...
        if start_positions is not None and end_positions is not None:
           # print("start original:",start_positions)
           # print("end origiinalL",end_positions)
